...
class ScanobjectnnDataset(PointCloudDataset):
    ...
    def __init__(self, config, train=True, orient_correction=True):
        ...
        # Dataset folder
        self.path = 'data/h5_files/main_split'
        ...
        self.input_points,self.input_labels = self.load_subsampled_clouds(orient_correction)
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        if exists(filename):
            with open(filename, 'rb') as file:
                input_points, input_labels = pickle.load(file)
        ...
        # Else compute them from original points
            ...
            input_points = []
            input_normals =None
            h5_name = 'data/h5_files/main_split/'+split+'_objectdataset_augmentedrot_scale75.h5'
           ...
           # Advanced display
            ...
            for point in data:
                if self.config.first_subsampling_dl > 0:
                    point = grid_subsampling(point,sampleDl=self.config.first_subsampling_dl)   
                    input_points += [point]
            input_labels = f['label'][:].astype('int64')       
            # Save for later use
...
class ScanobjectnnSampler(Sampler):
    """Sampler for ModelNet40"""

    def __init__(self, dataset:ScanobjectnnDataset, use_potential=True, balance_labels=False):
...
def ScanobjectnnCollate(batch_data):
    return ScanobjectnnCustomBatch(batch_data)
