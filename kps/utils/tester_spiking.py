...
class ModelTester:
        ...
        while np.min(self.test_counts) < num_votes:
            ...
            for batch in test_loader:
                ...
                with torch.no_grad():
                    ...
                    #spike_seq_monitor.enable()
                    # flop,para=profile(net,inputs =(batch, config))
                    # flop,para=clever_format([flop,para],"%.3f")
                    # print(flop)
                    #flops,param=count_flops_params(net,config,(11024,4))
                    #print(net)
                    outputs = net(batch, config)
                    
                    #print(f'spike_seq_monitor.records=\n{spike_seq_monitor.records}')
                    #self.average_of_matrices(spike_seq_monitor)
                    #spike_seq_monitor.disable()
                    #print(f'spike_seq_monitor.records=\n{spike_seq_monitor.records}')
    ...
    def average_of_matrices(self, matrix_list):
        averages = []
        for matrix in matrix_list:
            averages.append(torch.mean(matrix.cpu()))
        print(averages)
        # 绘制折线图
        plt.clf()
        plt.plot(averages)
        ...
        plt.ylabel('Average')
        plt.title('Averages of Matrices')
       
        plt.savefig('averages_plot.png')
        #plt.savefig(filename)  # 保存为PNG文件
        plt.show()
